...
OpenDGD is the openly-published format behind the dangerous-goods declaration software NCB Hazcheck has
run in production since 2015, built on the National Cargo Bureau's work on the safe carriage of cargo at
sea since 1952. v1.0 is the settled, stable release any vendor, carrier or authority can adopt.
...
[opendgd.org](https://opendgd.org) and [opendgd.com](https://opendgd.com) are served by the
render-service container (see [`Dockerfile`](Dockerfile)), which serves the built site and the API
together.
